...
In progress. Generating the raw fake data sets is complicated, but
this is how one would pre-process the raw HMP subject metadata.

```r

phenos_l = load_and_clean_phenotypes(config_l)
pres_df = prepare_abundance(config_l)  # presence-abscence
srs_map = prepare_map(config_l)

mappable_visnos = get_mappable_visnos(srs_map)
...
ph_vm = get_visno_mappable_pheno_df(phenos_l, mappable_visnos)
ph_vu = get_visno_unmappable_pheno_df(phenos_l, mappable_visnos)
...
In the meantime, please enjoy a random cleaned example.
